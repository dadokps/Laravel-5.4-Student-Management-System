   ...
   public function getStudentList()
   {
    ...
    public function showStudentInfo(Request $request)
    {
        $classes = $this->info()->select(DB::raw('students.student_id,
                                ...
                                ->where('classes.class_id', $request->class_id)->get();
    ...
    public function getStudentMultiList()
    {
        $academics  = Academic::orderBy('academic_id', "DESC")->get();
        $programs   = Program::all();
        $shifts     = Shift::all();
        ...
        return view('report.studentMultiList', compact('programs', 'academics', 'shifts', 'times', 'batches', 'groups'));
    }

    public function showStudentMultiInfo(Request $request)
    {
        if($request->ajax())
        {
            if(!empty($request['checkAll']))
            {
                $classes = $this->info()->select(DB::raw('students.student_id,
                                    CONCAT(students.first_name," ",students.last_name) as name,
                                    (CASE WHEN students.sex=0 THEN "Male" ELSE "Female" END) as sex,
                                    students.dob, programs.program ,levels.level, shifts.shift, times.time, batches.batch, groups.group '))
                                ->whereIn('classes.class_id', $request['checkAll'])
                                ->get();

                return view('report.studentInfoMultiList', compact('classes'));
